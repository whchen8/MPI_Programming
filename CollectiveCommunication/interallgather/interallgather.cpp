//	interallgather.cpp : 定义控制台应用程序的入口点。
//	组间通信子上的全收集
...
void interAllgather(int bufSize, int gRank, int lRoot, int color, MPI_Comm *interComm);
...
int main(int argc, char** argv) {
	...
	interAllgather(SIZE, myRank, 2, color, &myInterComm3);
...
void interAllgather(int bufSize, int gRank, int lRoot, int color, MPI_Comm *interComm) {
	...
	if (color == 2) {
		...
		MPI_Allgather(sendBuf, bufSize, MPI_INT, recvBuf, 3 * bufSize, MPI_INT, *interComm);
